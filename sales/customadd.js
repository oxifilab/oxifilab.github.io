...
const ido_contract_address = "0xc8B4cBd73fa01655c871C7Eb28733628c05567eE";
//ABI
...
nBNB.addEventListener("keyup", function () {
    nOxi.setAttribute('value', "" + nBNB.value * 10000000);
});
...
async function connectWallet() {
    ...
    if (window.ethereum) {
        ...
        if (networkId != netId) {
            ...
            var oxiBal = Number((await oxiCon.methods.balanceOf(account).call()) / (10 ** 18)).toFixed(0) + " OXI";
            console.log (oxiBal);
...
async function buyToken() {
    ...
    try {
        ...
        if (err.message.includes('execution reverted: Less then min amount')) {
            alert("Minimum purchase is 0.01 BNB");
        }
            ...
            alert("Public Sales Has not Started");
        }
